...
const routes = [
  { path: '/', redirect: () => resolveRoleDefaultRoute(getStoredRole()) },
  {
    ...
    children: [
      { path: '', name: 'AdminDashboard', component: AdminDashboard, meta: { role: ROLES.ADMIN, title: '平台工作台', section: '总览', description: '查看平台核心指标、审核进度与监管概况' } },
      ...
      { path: 'orders', name: 'AdminOrders', component: () => import('../views/orders/HistoryOrders.vue'), meta: { role: ROLES.ADMIN, title: '全局订单管理', section: '订单监管', description: '查看平台订单、状态流转与交易信息' } },
      { path: 'orders/abnormal', name: 'AdminOrderAbnormal', component: () => import('../views/orders/AbnormalOrders.vue'), meta: { role: ROLES.ADMIN, title: '异常订单管理', section: '订单监管', description: '查看平台异常订单及异常原因' } },
      { path: 'orders/anomalies', redirect: '/admin/orders/abnormal' },
      { path: 'orders/detail/:id', name: 'AdminOrderDetail', component: () => import('../views/orders/OrderDetailPage.vue'), meta: { role: ROLES.ADMIN, title: '订单详情', section: '订单监管', description: '查看订单的完整交易明细' } },
      ...
      { path: 'marketing', name: 'AdminMarketing', component: () => import('../views/admin/MarketingAudit.vue'), meta: { role: ROLES.ADMIN, title: '营销合规审核', section: '系统配置', description: '审核营销活动、优惠策略与平台规则' } },
      { path: 'settings', name: 'AdminSettings', component: () => import('../views/admin/PermissionControl.vue'), meta: { role: ROLES.ADMIN, title: '权限控制', section: '系统配置', description: '维护后台各模块的查看、编辑、审批和导出权限' } },
      { path: 'settings/params', redirect: '/admin/settings/params/basic' },
      { path: 'settings/params/basic', name: 'AdminSettingsParamsBasic', component: () => import('../views/admin/SystemParams.vue'), meta: { role: ROLES.ADMIN, title: '系统参数配置', section: '系统配置', description: '维护平台基础、计费、清分和通知参数', settingsSection: 'basic' } },
      { path: 'settings/params/billing', name: 'AdminSettingsParamsBilling', component: () => import('../views/admin/SystemParams.vue'), meta: { role: ROLES.ADMIN, title: '系统参数配置', section: '系统配置', description: '维护平台基础、计费、清分和通知参数', settingsSection: 'billing' } },
      { path: 'settings/params/settlement', name: 'AdminSettingsParamsSettlement', component: () => import('../views/admin/SystemParams.vue'), meta: { role: ROLES.ADMIN, title: '系统参数配置', section: '系统配置', description: '维护平台基础、计费、清分和通知参数', settingsSection: 'settlement' } },
      { path: 'settings/params/notification', name: 'AdminSettingsParamsNotification', component: () => import('../views/admin/SystemParams.vue'), meta: { role: ROLES.ADMIN, title: '系统参数配置', section: '系统配置', description: '维护平台基础、计费、清分和通知参数', settingsSection: 'notification' } },
  ...
  },
  {
    ...
    children: [
      ...
      { path: 'orders/history', name: 'OperatorOrdersHistory', component: () => import('../views/orders/HistoryOrders.vue'), meta: { role: ROLES.OPERATOR, title: '历史订单', section: '订单管理', description: '查看已完成订单和结算信息' } },
      { path: 'orders/realtime', name: 'OperatorOrdersRealtime', component: () => import('../views/orders/RealtimeOrders.vue'), meta: { role: ROLES.OPERATOR, title: '实时订单', section: '订单管理', description: '监控当前充电订单和实时状态' } },
      { path: 'orders/abnormal', name: 'OperatorOrdersAbnormal', component: () => import('../views/orders/AbnormalOrders.vue'), meta: { role: ROLES.OPERATOR, title: '异常订单', section: '订单管理', description: '查看异常订单并进行复核处理' } },
      { path: 'orders/detail/:id', name: 'OperatorOrderDetail', component: () => import('../views/orders/OrderDetailPage.vue'), meta: { role: ROLES.OPERATOR, title: '订单详情', section: '订单管理', description: '查看订单的完整交易明细' } },
