...
const routes = [
  { path: '/', redirect: () => resolveRoleDefaultRoute(getStoredRole()) },
  {
    ...
    children: [
      ...
      { path: 'orders', name: 'AdminOrders', component: () => import('../views/orders/HistoryOrders.vue'), meta: { role: ROLES.ADMIN, title: '全局订单查询', section: '订单监管', description: '查看全平台订单流水与交易数据' } },
      { path: 'orders/anomalies', name: 'AdminOrderAnomalies', component: () => import('../views/orders/AbnormalOrders.vue'), meta: { role: ROLES.ADMIN, title: '异常订单监管', section: '订单监管', description: '处理异常订单、告警和人工复核记录' } },
  ...
  },
  {
    ...
    children: [
      ...
      { path: 'orders', name: 'OperatorOrders', component: () => import('../views/orders/HistoryOrders.vue'), meta: { role: ROLES.OPERATOR, title: '历史订单', section: '订单管理', description: '查询历史订单、费用和充电记录' } },
